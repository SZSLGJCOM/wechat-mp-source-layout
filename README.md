...
- HTML 源码模式：点击工具栏 HTML 进入纵向格式化源码，再次点击自动保存并返回正文。
- 手机预览：在微信工具栏开启可拖动的 iPhone 阅读预览，编辑内容实时同步。
- 图片简单调节：点击文章图片即可进行圆角、尺寸、间距、阴影、发光、羽化、描边、色彩、透明度、相框、图注、圆形等参数微调；图片选中、拖动与缩放由微信编辑器原生处理。
- SVG 简单动效：支持选择 2 到 9 张图片生成淡入、擦除、滑入、轮播、叠入等效果动效。
